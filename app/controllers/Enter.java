 ...
 */
public class Enter extends Controller {
    /**
    ...
    public static Result estimator() {
		...
		// So, we are using BigDecimal instead of Long.
 		BigDecimal providerId = new BigDecimal(socialUser.id.id);
        models.Customer customer = models.Customer.findByProviderId(providerId);
        ...
        /**
 		...
 		String displayName = socialUser.firstName + " " + socialUser.lastName;
 		
		Logger.debug("id: " + socialUser.id.id);
		Logger.debug("fullName: " + displayName);
 	   ...
 	   if (customer == null) {
        	models.Customer newCustomer =  models.Customer.register(providerId, 
        												displayName,
        												socialUser.email, 
        	...
        	session("userId", Long.toString(newCustomer.id));
          	session("displayName", newCustomer.displayName);
